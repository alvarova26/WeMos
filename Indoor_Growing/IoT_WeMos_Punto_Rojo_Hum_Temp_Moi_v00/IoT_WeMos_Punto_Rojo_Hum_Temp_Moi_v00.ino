...
/* ################################################################################################### */
/* ############################## CHECK Note v00.h FOR DETAILS => libraries, values, behavior, etc. ## */
/* ################################################################################################### */
...
unsigned int length = 0;                          //Length of the incoming MQTT msg
...
const int AirValue = 790;                         //Value with the sensor measuring just air
const int WaterValue = 390;                       //Value with the sensor measuring jsut water
...
void callback(char* topic, byte* payload, unsigned int length);//FUNCTION OK
...
void setup_mqtt()
{
  ...
  mqttClient.publish(topicWeMosReadings, "SW: IoT_WeMos_Punto_Rojo_Hum_Temp_Ctrl_v00"); //Publish SW Version for version control
...
float mem_logic(float value_mea, bool mea_ini, char code)
{
  ...
  */
  ...
  {
    case 't':
      Serial.println("mem_logic option t."); //Print the auxiliar text to Serial Comm (USB)
      if (mea_ini == 1)
      {
        for (i = 0; i < 5; i++)
