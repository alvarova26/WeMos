...
bool airflow_stat = 0;                            //Code for follow up the status of the air flow - if received, calls set_airflow();
...
void publish_readings()
{
  ...
  JSONencoder["ALILLP"] = String (alt_ill_per);
  JSONencoder["ALILLY"] = String (alt_ill_cyc);
  JSONencoder["ALILLC"] = String (alt_ill_count);

  // Publish Air Flow Status
  JSONencoder["AIRSTA"] = String (airflow_stat);

  // Publish Relay Status
  JSONencoder["LEDAST"] = String (LED_A_stat);
  JSONencoder["LEDBST"] = String (LED_B_stat);
  JSONencoder["LEDCST"] = String (LED_C_stat);
  JSONencoder["FANAST"] = String (FAN_A_stat);
...
void update_WeMos(String inputmsg)
{
  ...
  else
  {
    ...
    {
      reset_LED_A_relay = root["LDA_RESET"];
    ...
    {
      reset_LED_B_relay = root["LDB_RESET"];
    ...
    {
      reset_LED_C_relay = root["LDC_RESET"];
    ...
    {
      reset_FAN_A_relay = root["FNA_RESET"];
...
void check_illum()
{
  ...
  if (illumin_stat == 9)                          //Alternate LOW Illumination Levels
  {
    if (alt_ill_stat)
    {
      Serial.println("======================================== WeMos Illumination Set to MIDA | Begin");
...
{
  Serial.println("dentro de clear_mem");
