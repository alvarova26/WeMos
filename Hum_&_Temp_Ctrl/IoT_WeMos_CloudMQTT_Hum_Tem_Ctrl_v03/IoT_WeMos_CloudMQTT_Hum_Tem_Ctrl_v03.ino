...
// Important!!!!!!!!!!!!!!!!!!!!
// MQTT_MAX_PACKET_SIZE defined in the library header MUST BE INCREASED to allow larger payloas in client.publish() method
...
int ref_stat = 0; //Refrigerator's status
...
int hum_stat = 0; //Humidifier's status
double hum_UP = 0;
double hum_DOWN = 0;
...
char par = 'u'; //Code for printing (b=before or a=after)
...
bool flag = 1; //Flag for init system
...
float mem_logic(float value, bool flag, char measure);
void print_sensor();
...
void publish_readings()
{
  StaticJsonBuffer<500> JSONbufferRX;
  ...
  JSONencoder["TEMPMEM"] = String (temp_VALUE_mem);
  ...
  // Hum parameters
  JSONencoder["HUMMEM"] = String (hum_VALUE_mem);
